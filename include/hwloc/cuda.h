 ...
 */
static __hwloc_inline int
hwloc_cuda_get_device_cpuset(hwloc_topology_t topology, CUdevice cudevice,
                             hwloc_cpuset_t set)
{
  ...
  cres =
      cuDeviceGetAttribute(&numaId, CU_DEVICE_ATTRIBUTE_HOST_NUMA_ID, cudevice);
  if (cres != CUDA_SUCCESS || numaId < 0) {
    errno = ENOSYS;
    return -1;
  }
